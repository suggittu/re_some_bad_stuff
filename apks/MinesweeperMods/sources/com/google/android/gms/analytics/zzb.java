...
public class zzb extends com.google.android.gms.analytics.internal.zzc implements zzi {
    ...
    private static String zzH(Map map) {
    ...
    public static Map zzc(com.google.android.gms.measurement.zzc zzcVar) {
        HashMap map = new HashMap();
        zzkd zzkdVar = (zzkd) zzcVar.zze(zzkd.class);
        if (zzkdVar != null) {
            for (Map.Entry entry : zzkdVar.zziR().entrySet()) {
        ...
        if (zzkbVar != null) {
            for (Map.Entry entry2 : zzkbVar.zziP().entrySet()) {
                String strZzU = zzc.zzU(((Integer) entry2.getKey()).intValue());
        ...
        if (zzkcVar != null) {
            for (Map.Entry entry3 : zzkcVar.zziQ().entrySet()) {
                String strZzW = zzc.zzW(((Integer) entry3.getKey()).intValue());
                if (!TextUtils.isEmpty(strZzW)) {
                    map.put(strZzW, zzb(((Double) entry3.getValue()).doubleValue()));
                }
            }
        }
        ...
        if (zzptVar != null) {
            ...
            if (productActionZzAV != null) {
                for (Map.Entry entry4 : productActionZzAV.build().entrySet()) {
                    if (((String) entry4.getKey()).startsWith("&")) {
                        map.put(((String) entry4.getKey()).substring(1), entry4.getValue());
                    } else {
                        map.put(entry4.getKey(), entry4.getValue());
                    }
                }
            }
            Iterator it = zzptVar.zzAY().iterator();
            int i = 1;
            while (it.hasNext()) {
                map.putAll(((Promotion) it.next()).zzba(zzc.zzaa(i)));
                i++;
            }
            Iterator it2 = zzptVar.zzAW().iterator();
            int i2 = 1;
            while (it2.hasNext()) {
                map.putAll(((Product) it2.next()).zzba(zzc.zzY(i2)));
                i2++;
            }
            int i3 = 1;
            for (Map.Entry entry5 : zzptVar.zzAX().entrySet()) {
                List list = (List) entry5.getValue();
                String strZzad = zzc.zzad(i3);
                Iterator it3 = list.iterator();
                int i4 = 1;
                while (it3.hasNext()) {
                    map.putAll(((Product) it3.next()).zzba(strZzad + zzc.zzab(i4)));
                    i4++;
                }

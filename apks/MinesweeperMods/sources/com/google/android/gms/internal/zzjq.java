...
public class zzjq extends WebViewClient {
    ...
    public final void onReceivedSslError(WebView webView, SslErrorHandler sslErrorHandler, SslError sslError) {
        if (sslError != null) {
            ...
            zza(this.zzpD.getContext(), "ssl_err", (primaryError < 0 || primaryError >= zzNz.length) ? String.valueOf(primaryError) : zzNz[primaryError], com.google.android.gms.ads.internal.zzr.zzbE().zza(sslError));
        }
    ...
    public void zza(String str, zzdf zzdfVar) {
        synchronized (this.zzpV) {
            List copyOnWriteArrayList = (List) this.zzNA.get(str);
    ...
    public void zzh(Uri uri) {
        ...
        List list = (List) this.zzNA.get(path);
        if (list == null) {
            zzin.v("No GMSG handler found for GMSG: " + uri);
            return;
        }
        Map mapZze = com.google.android.gms.ads.internal.zzr.zzbC().zze(uri);
        if (zzin.zzQ(2)) {
            zzin.v("Received GMSG: " + path);
            for (String str : mapZze.keySet()) {
                zzin.v("  " + str + ": " + ((String) mapZze.get(str)));
            }
        }
        Iterator it = list.iterator();
        while (it.hasNext()) {
            ((zzdf) it.next()).zza(this.zzpD, mapZze);
        }
    }

...
public class zzl {
        ...
        if (zzkVar.zzr()) {
            synchronized (this.zzZ) {
                ...
                if (this.zzZ.containsKey(strZzh)) {
                    Queue linkedList = (Queue) this.zzZ.get(strZzh);
                    if (linkedList == null) {
    ...
    void zzf(zzk zzkVar) {
        ...
        synchronized (this.zzaf) {
            Iterator it = this.zzaf.iterator();
            while (it.hasNext()) {
                ((zza) it.next()).zzg(zzkVar);
            }
        }
        if (zzkVar.zzr()) {
            synchronized (this.zzZ) {
                String strZzh = zzkVar.zzh();
                Queue queue = (Queue) this.zzZ.remove(strZzh);
                if (queue != null) {
                    if (zzs.DEBUG) {
                        zzs.zza("Releasing %d waiting requests for cacheKey=%s.", Integer.valueOf(queue.size()), strZzh);
                    }

...
class zzb extends RelativeLayout {
    ...
    public zzb(Context context, zza zzaVar) {
        ...
        List listZzdG = zzaVar.zzdG();
        if (listZzdG.size() > 1) {
            this.zzxT = new AnimationDrawable();
            Iterator it = listZzdG.iterator();
            while (it.hasNext()) {
                this.zzxT.addFrame((Drawable) it.next(), zzaVar.zzdH());
            }
            zzr.zzbE().zza(imageView, this.zzxT);
        } else if (listZzdG.size() == 1) {
            imageView.setImageDrawable((Drawable) listZzdG.get(0));
        }

    ...
    final void draw(Canvas canvas) {
        ...
        if (this.mRunning || this.mFinishTime > 0) {
            ...
            if (this.mRunning) {
                ...
                float interpolation = INTERPOLATOR.getInterpolation((((jCurrentAnimationTimeMillis - this.mFinishTime) % 1000) / 10.0f) / 100.0f) * i;
                this.mClipRect.set(i - interpolation, 0.0f, interpolation + i, iHeight);
            ...
            if (f >= 0.0f && f <= 25.0f) {
                drawCircle(canvas, i, i2, this.mColor1, ((25.0f + f) * 2.0f) / 100.0f);
            }
            if (f >= 0.0f && f <= 50.0f) {
                drawCircle(canvas, i, i2, this.mColor2, (2.0f * f) / 100.0f);
            }
            if (f >= 25.0f && f <= 75.0f) {
